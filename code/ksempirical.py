...
try:
    symbol_data = my_share.get_historical(share.PERIOD_TYPE_YEAR,6.1,
...
empirical = price3[period_fix]
ks_value = []
for i in range(len(price3)-period_fix-1):
    empirical2 = price3[period_fix+i+1]
    ks_value.append(stats.ks_2samp(empirical,empirical2).statistic)
    empirical = empirical2

date4 = []
for i in range(len(price3)-period_fix):
    date4.append(date2[period_fix+i])


print(len(date4), len(ks_value))
# for i in range(len(ks_value)):
#     ks_value[i] = ks_value[i] + 1e-4

# ks_ave = np.ones(len(ks_value))
# for i in range(len(ks_value)-4):
#     ks_ave[i+2] = (ks_value[i] + ks_value[i+1] + ks_value[i+2] + ks_value[i+3] + ks_value[i+4]) / 5

# ks_log = []
# for i in range(len(ks_value)):
#     ks_log.append(np.log(ks_value[i]))

# fig = plt.figure()
# ax1 = fig.add_subplot(111)
# ln1=ax1.plot(date3,ks_value, color='m', label='ks value')
