...
print(date2[0], date2[len(date2)-1])
...
for i in range(period):
    logreturn2.append(float(logreturn[i]))


alpha_trans = []
beta_trans = []
date3 = []
for i in range(len(logreturn)-period):
    para = estpar(logreturn2)
    alpha_trans.append(float(para[0]))
    beta_trans.append(float(para[1]))
    logreturn2.append(float(logreturn[period+i]))
    del logreturn2[0]
    date3.append(date2[period+i])

price2 = []
for i in range(len(logreturn)-period):
    price2.append(price3[period+i])

print(date3[0], date3[len(date3)-1])

stable = stable_random(alpha_trans[0], beta_trans[0])
ks_value = []
for i in range(len(alpha_trans)-1):
    stable2 = stable_random(alpha_trans[i+1], beta_trans[i+1])
...
#logreturn3 = []
#for i in range(len(logreturn)-period):
    #logreturn3.append(logreturn[period+i])
