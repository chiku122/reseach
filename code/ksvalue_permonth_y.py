...
def stable_random(alpha, beta, gamma, delta):
    ...
    if(alpha-1.0 < 0.01):
        for i in range(N):
            ...
            y[i] = gamma*x[i]+2/np.pi*beta*gamma*np.log(gamma)+delta
...
try:
    symbol_data = my_share.get_historical(share.PERIOD_TYPE_YEAR,17.5,
...
logreturn = np.zeros(len(price3)-1)
...
# stable = stable_random(alpha_ave[0], beta_ave[0])
# ks_value = []
# for i in range(len(alpha_ave)-1):
#     stable2 = stable_random(alpha_ave[i+1], beta_ave[i+1])
#     ks_value.append(stats.ks_2samp(stable,stable2).statistic)
#     stable = stable2

# ks_value.append(0)

stable = stable_random(alpha_ave[0], beta_ave[0], gamma_ave[0], delta_ave[0])
ks_value = []
interval = 10
k = 0
while((k+1)*interval < len(alpha_ave)):
    stable2 = stable_random(alpha_ave[(k+1)*interval], beta_ave[(k+1)*interval], gamma_ave[(k+1)*interval], delta_ave[(k+1)*interval])
    ks_value.append(stats.ks_2samp(stable,stable2).statistic)
    stable = stable2
    k = k + 1
...
date4 = []
for i in range(len(ks_value)):
    date4.append(date3[i*interval])

fig = plt.figure()
ax1 = fig.add_subplot(111)
ln1=ax1.plot(date4,ks_value, color='m', label='ks value')
