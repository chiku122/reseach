...
try:
    symbol_data = my_share.get_historical(share.PERIOD_TYPE_YEAR,6.5,
...
betaave = np.sum(beta_trans,axis=0)
betaave = betaave / 6

betamask = np.ma.masked_where(alphaave > 1.6, betaave)
...
with open('partransitionbeta.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(date3)
    writer.writerow(betamask)
    writer.writerow(price2)
...
ln1=ax1.plot(date3,betamask, color='b', label='beta')
